        ...
        for epoch in iters:
            ...
            for view_idx, logit in enumerate(logits):
                if xent_loss is None:
                    xent_loss = b_xent(logit, lbl)+F.binary_cross_entropy_with_logits(x_proList[view_idx],torch.squeeze(features[view_idx])) #+F.mse_loss(x_proList[view_idx],torch.squeeze(features[view_idx]))
                else:
                    xent_loss += b_xent(logit, lbl)+F.binary_cross_entropy_with_logits(x_proList[view_idx],torch.squeeze(features[view_idx])) #+F.mse_loss(x_proList[view_idx],torch.squeeze(features[view_idx]))
