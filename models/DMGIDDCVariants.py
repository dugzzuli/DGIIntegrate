...
class DMGIDDCVariants(embedder):
    ...
    def training(self,f):
        ...
        if(self.args.Fine):
            for epoch in iters:
                ...
                y, h, h_1_all=result["y"],result["h"],result["h_1_all"]

                # clustering_loss = cls_criterion(h, h_1_all)
                clustering_loss = cls_criterion(y, h_1_all)


                y_, h_, h_2_all = result["y_"], result["h_"], result["h_2_all"]
                #
                clustering_loss_neg = cls_criterion(y_, h_)

                loss+=self.args.cls_reg*clustering_loss +0.1*clustering_loss_neg
                if loss < best:
                    best = loss
                    cnt_wait = 0
                ...
                if(epoch%10)==0:
                    ...
                    with torch.no_grad():
                        ...
                        y_pred_ = temp_result["y_"].detach().cpu().max(1)[1]
                        try:
                            acc,nmi,ari,stdacc,stdnmi,stdari=eva(np.array(y_pred),train_lbls, str(epoch) + 'Q',Flag=False)
                            eva(np.array(y_pred_), train_lbls, str(epoch) + 'Q',Flag=True)
                        except:
                            pass
                        else:
                            pass
                    ...
                    retxt = "loss:{} epoch:{} acc:{} accMax:{}".format(
                        loss.item(), epoch, acc, accMax)
...
class modeler(nn.Module):
    ...
    def forward(self, feature, adj, shuf, sparse, msk, samp_bias1, samp_bias2):
        ...
        result["h_2_all"] = h_2_all
        y_, h_ = self.clustering_module(h_2_all)
        result["y_"]=y_
        result["h_"]=h_
